			...
			jQuery.each(cart, function(key, value) {
				if (key === 'items') {
					...
					if (value.length) {   
						...
						jQuery('<div class="pd"><div class="top-subtotal">Tổng tiền: <span class="price">' + Haravan.formatMoney(cart.total_price, "{{amount_no_decimals_with_comma_separator}}₫") + '</span></div></div>').appendTo(table);
			...
			jQuery.each(cart, function(key, value) {
				if (key === 'items') {
					...
					if (value.length) {  
						...
						+ '<tfoot><tr><td colspan="20" class="a-right"></td><td class="a-right"><span class="total_tt">Tổng số thành tiền:</span> <strong><span class="totals_price price">' + Haravan.formatMoney(cart.total_price, "{{amount_no_decimals_with_comma_separator}}₫") + '</span></strong></td></tr></tfoot></table>'
						...
						jQuery.each(value, function(i, item) {
							...
							if(link_img1=="null" || link_img1 =='' || link_img1 ==null){
								link_img1 = 'https://hstatic.net/0/0/global/noDefaultImage6_large.gif';
							}
							if(item.price == 0){
								var price = "Tặng kèm";
								var hidden = "hidden";
							...
							+ '<div style="width: 10%" class="image cart1"><a class="product-image" title="' + item.title + '" href="' + item.url + '"><img width="120" height="auto" alt="' + item.title + '" src="' + link_img1 +  '"></a></div>'
							+ '<div style="width: 33%;align-items: flex-start;" class="a-center cart2"><h2 class="product-name" title="'+ item.title +'"> <a href="' + item.url + '">' + item.title + '</a><span class="variant-title">' + item.variant_options + '</span> </h2><div style="height: 30px;position: relative;width: 78px;padding: 10px 0;"></div></div><div style="width: 19%" class="a-center"><span class="item-price"> <span class="price pricechange">' + price + '</span></span></div>'
							...
							+ '<div style="width: 9%;align-items: flex-end;padding-right: 0;" class="a-center cart6"><span class="cart-price"> <span class="price">'+ Haravan.formatMoney(item_price, "{{amount_no_decimals_with_comma_separator}}₫") +'</span> </span></div>'
							+ '</div>';
					...
					if (key === 'items') {
						...
						if (value.length) { 
							jQuery.each(value, function(i, item) {
								...
								+ '<a class="product-image" href="' + item.url + '" title="' + item.title + '"><img alt="'+  item.title  + '" src="' + src +  '"width="'+ '90' +'"\></a>'
								+ '</div></div>'
								+ '<div style="width:39.8%;" class="height text-left"><div class="item-info"><p class="item-name"><a class="text2line" href="' + item.url + '" title="' + item.title + '">' + item.title + '</a></p>'
								+ '<span class="variant-title-popup">' + item.variant_options + '</span>'
								+ '<a href="javascript:;" class="remove-item-cart" title="Xóa" data-id="'+ item.variant_id +'"><i class="fas fa-times"></i>&nbsp;&nbsp;Xoá</a>'
								+ '<p class="addpass" style="color:#fff;margin:0px;">'+ item.variant_id +'</p>'
								+ '</div></div>'
								+ '<div style="width: 14.9%;" class="border height text-center"><div class="item-price"><span class="price pricechange">' + Haravan.formatMoney(item.price, "{{amount_no_decimals_with_comma_separator}}₫") + '</span>'
								...
								+ '<div style="width: 17.2%;" class="border height text-center"><span class="cart-price"> <span class="price">'+ Haravan.formatMoney(item.price * item.quantity, "{{amount_no_decimals_with_comma_separator}}₫") +'</span> </span></div>'
								+ '</div>';
						...
						if (value.length) {   
							...
							jQuery.each(value, function(i, item) {
								...
								jQuery('<div class="item-product item productid-' + item.variant_id +' "><div class="item-product-cart-mobile"><a href="' + item.url + '">	<a class="product-images1" href=""  title="' + item.title + '"><img width="80" height="150" src="' + src +  '" alt="' + item.title + '"></a></a></div>'
									+ '<div class="title-product-cart-mobile"><h3><a href="' + item.url + '" title="' + item.title + '">' + item.title + '</a></h3><p><span class="pricechange">' + Haravan.formatMoney(item.price, "{{amount_no_decimals_with_comma_separator}}₫") + '</span></p></div>'
